const WORKER_URL = (typeof import.meta !== "undefined" && (import.meta as any).env?.VITE_WORKER_URL) || "http://localhost:8787";
...
// Generate API
async function submitGenerate(generationId: string) {
  return request<{ success: boolean }>("/api/generate", {
    method: "POST",
    body: { generationId } as any,
  });
}
